//
//  FMWarehouseScanViewController.m
//  FlashMan
//
//  Created by 小河 on 17/1/22.
...
@interface FMWarehouseScanViewController ()<UITabBarDelegate,AVCaptureMetadataOutputObjectsDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
@implementation FMWarehouseScanViewController
// 界面显示,开始动画
...
-(void)addSubViews
{
    ...
    self.customLabel=[XHView createLabelWithFrame:CGRectMake(0, CGRectGetMaxY(_customContainerView.frame)+10,ScreenWidth, 30) text:@"将条码/二维码放入框内，将自动扫码，扫码成功即取货" textColor:[UIColor whiteColor] font:[UIFont systemFontOfSize:14.0f] superView:self.view];
...
-(void)postGetGoodsDataWithOrderId:(NSString *)orderId {
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@",SERVER_ADDRESS,kBINDWAREHOUSE];
    ...
    [request NetRequestPOSTWithRequestURL:url WithParameter:dict WithReturnValeuBlock:^(id returnValue) {
        ...
        [XHView showTipHud:@"确认失败" superView:self.view];
